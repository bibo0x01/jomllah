...
export class OffersService {
    ...
    async counterOffer(senderId, parentOfferId, data) {
        const parentOffer = await this.offersRepo.getOfferById(parentOfferId);
        if (!parentOffer)
            throw new NotFoundError('Parent offer not found');
        if (parentOffer.status !== 'PENDING')
            throw new ValidationError('Can only counter PENDING offers');
        if (parentOffer.receiverId !== senderId)
            throw new ForbiddenError('Only the receiver can counter this offer');
    ...
    async acceptOffer(userId, offerId) {
        const offer = await this.offersRepo.getOfferById(offerId);
        if (!offer)
            throw new NotFoundError('Offer not found');
        if (offer.status !== 'PENDING')
            throw new ValidationError('Can only accept PENDING offers');
        if (offer.receiverId !== userId)
            throw new ForbiddenError('Only the receiver can accept this offer');
    ...
    async rejectOffer(userId, offerId) {
        const offer = await this.offersRepo.getOfferById(offerId);
        if (!offer)
            throw new NotFoundError('Offer not found');
        if (offer.status !== 'PENDING')
            throw new ValidationError('Can only reject PENDING offers');
        if (offer.receiverId !== userId)
            throw new ForbiddenError('Only the receiver can reject this offer');
