...
export class OrdersService {
  ...
  async createOrderFromOffer(offer: any) {
    ...
    if (!chatRoom) throw new Error('ChatRoom not found for offer');
  ...
  async updateOrderStatus(userId: string, tenantId: string | undefined, orderId: string, status: OrderStatus) {
  ...
  async getMyOrders(userId: string, role: string, tenantId?: string) {
    if (role === 'BUYER') {
