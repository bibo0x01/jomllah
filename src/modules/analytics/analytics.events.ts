...
export const setupAnalyticsEvents = () => {
  ...
  eventBus.on('remote.chat.created', async (payload: { chatRoomId: string, tenantId: string }) => {
    try {
      await analyticsService.incrementTenantMetric(
        payload.tenantId, 
        'chats_opened', 
        `chat_opened_${payload.chatRoomId}`
      );
    } catch (error) {
  ...
  eventBus.on('remote.order.created', async (payload: { order: any }) => {
    try {
      await analyticsService.incrementTenantMetric(
        payload.order.tenantId, 
        'orders_completed', 
        `order_created_${payload.order.id}`
      );
    } catch (error) {
